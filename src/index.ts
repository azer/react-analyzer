...
import { ReactProps, PropType, ReactFile, Result, ReactComponent, FunctionDeclaration, ReactProp, FunctionParameterType, TypeDeclaration } from './types';
...
import { convertPropTreeToReactProps, defaultPropsVisitor, scanPropsUsage } from './props';
...
function createReactComponent(result: Result, func: FunctionDeclaration): ReactComponent {
    // @ts-ignore
...
function resolveUtilityType(result: Result, type: TypeDeclaration): TypeDeclaration {
...
function resolveReferences(result: Result, obj: any, seenTypes: Set<string> = new Set()): any {
    ...
    if (obj.type === 'type_reference') {
        ...
        if (referencedType) {
            ...
            const resolvedProps = resolveReferences(result, referencedType.props, new Set(seenTypes));
            return !obj.optional ? {
                type: 'object',
                props: resolvedProps,
...
export function resolveType(result: Result, type: TypeDeclaration): TypeDeclaration {
    return resolveReferences(result, type);
}

export function resolveFunction(result: Result, func: FunctionDeclaration): FunctionDeclaration {
